<div class="flex flex-col bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden h-[600px] shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
    ...
    x-init="scrollToBottom(); $wire.on('scroll-to-bottom', () => { setTimeout(() => { scrollToBottom() }, 50); });">

    {{-- Header --}}
    <div class="px-5 py-3.5 bg-white border-b border-slate-200 flex items-center justify-between shrink-0 z-10 shadow-sm">
        <div class="flex items-center gap-3">
            <div class="relative flex items-center justify-center w-10 h-10 rounded-xl bg-[#5b5ef4] text-white shadow-sm">
            ...
            <div>
                ...
                {{-- Status Admin Online --}}
                @if ($ticket->status !== 'closed')
                    <div id="admin-status-badge">
                        @if ($isAdminOnline)
                            <div class="flex items-center gap-1.5 mt-0.5">
                                <div class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
                                <p class="text-[11px] font-medium text-emerald-600">
                                    Support Online • {{ $lastAdminName }}
                                </p>
                            </div>
                        @else
                            <div class="flex items-center gap-1.5 mt-0.5">
                                <div class="w-2 h-2 bg-amber-500 rounded-full"></div>
                                <p class="text-[11px] font-medium text-amber-600">Menunggu Admin Online</p>
                            </div>
                        @endif
                    </div>
                @else
        ...
        <div class="flex items-center gap-2">
            <span class="text-[11px] font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full border border-slate-200">
    ...
    <div x-ref="chatContainer" class="flex-1 overflow-y-auto p-5 space-y-4 scroll-smooth">
        @forelse ($messages as $msg)
            <div class="flex flex-col {{ $msg['sender_type'] === 'user' ? 'items-end' : 'items-start' }} animate-fade-in">
                <div class="flex items-end gap-2 max-w-[85%] {{ $msg['sender_type'] === 'user' ? 'flex-row-reverse' : 'flex-row' }}">

                    {{-- Avatar --}}
                    @if ($msg['sender_type'] === 'admin')
                        <div class="w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center text-xs font-bold bg-[#5b5ef4] text-white shadow-sm">
                            {{ $msg['sender_initial'] ?? strtoupper(substr($msg['sender_name'], 0, 2)) }}
                        </div>
                    @else
                        <div class="w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center text-xs font-bold bg-slate-300 text-slate-600 shadow-sm">
                            {{ strtoupper(substr($msg['sender_name'] ?? 'U', 0, 2)) }}
                        </div>
                    @endif

                    {{-- Bubble --}}
                    <div class="relative group px-4 py-2.5 shadow-sm
                        ...
                        @else
                            <p class="text-[10px] font-bold text-indigo-200 mb-0.5">{{ $msg['sender_name'] ?? 'Pelanggan' }}</p>
                        @endif

                        <p class="text-[13px] leading-relaxed whitespace-pre-wrap break-words">{{ $msg['message'] }}</p>

                        <div class="flex items-center justify-end gap-1 mt-1">
                            <span class="text-[9px] {{ $msg['sender_type'] === 'user' ? 'text-indigo-200' : 'text-slate-400' }}">
    ...
    @if ($ticket->status !== 'closed')
        <div class="p-3 bg-white border-t border-slate-200 shrink-0">
            <form wire:submit.prevent="sendMessage" class="flex items-end gap-2 relative">
                <div class="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2 focus-within:border-[#5b5ef4] focus-within:ring-2 focus-within:ring-indigo-100 transition-all shadow-inner">
                    <textarea wire:model="message" wire:keydown.enter.prevent="sendMessage" maxlength="2000"
                        ...
                        rows="1" x-data="{ resize() { $el.style.height = '40px'; $el.style.height = $el.scrollHeight + 'px' } }"
...
<script>
    document.addEventListener('livewire:initialized', () => {
        ...
        // Update admin status real-time
        Livewire.on('admin-status-updated', (data) => {
                ...
                if (data.isOnline) {
                    statusBadge.innerHTML = `
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <div class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
                            <p class="text-[11px] font-medium text-emerald-600">
                                Support Online • ${data.adminName || 'Admin'}
                            </p>
                        </div>
                    `;
                } else {
