...
>
    ...
    <div x-ref="chatContainer" class="flex-1 overflow-y-auto p-5 space-y-4 scroll-smooth">
        @forelse ($messages as $msg)
            <div class="flex flex-col {{ $msg['sender_type'] === 'admin' ? 'items-end' : 'items-start' }} animate-fade-in">
                <div class="flex items-end gap-2 max-w-[85%] {{ $msg['sender_type'] === 'admin' ? 'flex-row-reverse' : 'flex-row' }}">
                    ...
                    <div class="relative group px-4 py-2.5 shadow-sm
                        ...
                        <div class="flex items-center justify-end gap-1 mt-1">
                            <span class="text-[9px] {{ $msg['sender_type'] === 'admin' ? 'text-indigo-200' : 'text-slate-400' }}">
    ...
    @if ($ticket->status !== 'closed')
        <div class="p-3 bg-white border-t border-slate-200 shrink-0">
            <form wire:submit.prevent="sendMessage" class="flex items-end gap-2 relative">
                ...
                <div class="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2 focus-within:border-[#5b5ef4] focus-within:ring-2 focus-within:ring-indigo-50 transition-all shadow-inner">
                    <textarea
                        wire:model="message"
...
<script>
    window.addEventListener('beforeunload', () => {
        @this.call('setAdminOffline');
    });

    document.addEventListener('visibilitychange', () => {
        if (!document.hidden) {
            @this.call('setAdminOnline');
        }
    });
